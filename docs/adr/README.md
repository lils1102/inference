...
| `0003-agent_native_is_optional_metadata_layer.md` | Accepted | Agent-aware 能力通过可选 metadata / SDK / adapter 增强 |
...
2. ADR-0009 是 Proposed，必须写成 benchmark-driven default，而不是绝对规则。
...
6. 所有 SOTA claim 必须绑定同等硬件、模型、精度、workload、SLO 和 benchmark 指标。

## 更新规则

ADR 的状态变更必须通过独立 PR，并说明影响的 docs、benchmark plan、模块任务和 Phase 1/Phase 2 边界。
